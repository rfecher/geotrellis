...
class TileFeatureSeqMergeSpec
    extends FunSpec
    with Matchers {

  val ones = IntArrayTile.fill(1, 10, 10)
  val twos = IntArrayTile.fill(2, 10, 10)
  val threes = List(IntArrayTile.fill(3, 10, 10), IntArrayTile.fill(3, 10, 10))
  val sevens = List(IntArrayTile.fill(7, 10, 10), IntArrayTile.fill(7, 10, 10))
  ...
  describe("The merge method") {
    ...
    it("data should have as many tiles as the least of the inputs") {
      ...
      ot3.data.size should be (2)
      ot4.data.size should be (2)
    }
    ...
    it("data should interoperate with TileFeature[Tile,Tile]") {
      ...
      ot3.data.size should be (1)
      ot4.data shouldBe a [Tile]
    }

    it("data should interoperate with TileFeature[Tile,MultibandTile]") {
      val ot1 = TileFeature(ones, threes) // two bands
      val ot2 = TileFeature(ones,
      ...
      ot3.data.size should be (2)
      ot4.data shouldBe a [MultibandTile]
